"""
Abstraction layer over OS-dependent byte streams
"""
import sys
import os
import socket
...
# Jython fix (Java does not allow multiplexing of blocking sockets)
if hasattr(select, 'cpython_compatible_select'):
    from select import cpython_compatible_select as select
else:
    from select import select

from rpyc.lib.compatibility import safe_import
...
class PipeStream(Stream):
    ...
    def __init__(self, incoming, outgoing):
        outgoing.flush()                    #This is not in 3.1
    ...
    def read(self, count):
        ...
        try:
            while count > 0:
                #3.1 uses buf = self.incoming.read(min(self.MAX_IO_CHUNK, count))
    ...
    def write(self, data):
        try:
            while data:
                ...
                data = data[written:]
            #3.1 has self.outgoing.flush() here
        except EnvironmentError, ex:
            self.close()
            raise EOFError(ex)
